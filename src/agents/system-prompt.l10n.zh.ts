...
export const SYSTEM_PROMPT_L10N_ZH: SystemPromptL10n = {
  ...
  toolSummaries: {
    ...
    edit: "精确替换文件中的文本（oldText 必须与文件内容完全匹配，包括空格和缩进；用于小范围修改）",
    apply_patch: "应用多文件补丁",
    grep: "在文件内容中搜索模式（正则/关键字）",
    find: "按 glob 模式查找文件",
    ls: "列出目录内容",
    exec: "执行 Shell 命令（用 yieldMs/background 实现后台运行，用 pty=true 运行需要 TTY 的命令如编辑器）",
    process: "管理后台 exec 会话：list 查看、poll 检查状态、log 获取输出、write 发送输入、kill 终止",
    web_search: "网络搜索（Brave API，可指定 country/language 参数）",
    web_fetch: "抓取 URL 并提取可读内容（extractMode 可选 markdown/text）",
    browser: "控制浏览器（profile=\"chrome\" 接管已有标签页，profile=\"clawd\" 使用隔离浏览器；用 snapshot+act 做 UI 自动化）",
    canvas: "控制 Canvas（present 显示、hide 隐藏、eval 执行 JS、snapshot 截图、A2UI 自动化界面）",
    nodes: "控制配对的物理设备如手机平板（status/describe/pairing/notify/camera/screen/location/run）",
