import type { TaskDecompositionPromptsL10n } from "./prompts.l10n.types.js";

/**
 * 智能任务分解系统的中文提示词
 */
export const TASK_DECOMPOSITION_PROMPTS_ZH: TaskDecompositionPromptsL10n = {
  // ========================================
  // 任务分解提示词
  // ========================================
  
  decompositionExpertRole: "你是一个资深的任务分解专家，擅长将复杂任务拆解为结构清晰、可独立执行的子任务。",
  
  decompositionInstruction: "请将以下任务分解为 2-8 个可执行的子任务。分解时要考虑任务间的依赖关系、并行执行的可能性、以及每个子任务的量化产出要求。",
  
  rootTaskLabel: "根任务",
  
  currentTaskLabel: "当前任务",
  
  taskIdLabel: "ID",
  
  taskDescriptionLabel: "描述",
  
  taskDepthLabel: "深度",
  
  decompositionRequirementsTitle: "分解要求：",
  
  decompositionRequirements: [
    "子任务数量：2-8 个",
    "每个子任务应该是独立可执行的，尽量减少子任务间的依赖关系以支持并行执行",
    "子任务应该覆盖当前任务的所有要求，不遗漏任何关键步骤",
    "子任务之间可以有依赖关系（通过 dependencies 字段指定），但无依赖的任务应该能并行执行",
    "子任务的粒度应该适中（不要过于细碎或过于粗糙）",
    "如果原始任务有字数/篇幅/数量等量化要求，必须在每个子任务的 prompt 中明确分配具体的量化指标（如：本章要求 3000 字以上）",
    "每个子任务的 prompt 应该足够详细，包含：目标、具体要求、量化指标、输出格式等",
    "写作类任务：每个子任务的 prompt 中必须包含明确的字数要求，且所有子任务的字数之和应 >= 总要求",
    "⚠️ 章节拆分原则：即使用户把多个章节写在一起（如'第5-6章'或'第3、4章'），也必须拆分为独立的子任务（第5章一个子任务、第6章一个子任务）。每个章节必须是一个独立子任务，绝不合并",
    "⚠️ 能力约束：单个子任务的预期输出不应超过 2000 字（中文字符）。如果某个部分需要更多内容，必须拆分为多个子任务，通过依赖关系串联保证连贯性",
  ],
  
  jsonFormatInstruction: "请按照以下 JSON 格式返回分解结果：",
  
  jsonOnlyReminder: "请只返回 JSON 对象，不要包含其他内容。",
  
  // ========================================
  // 失败经验学习提示词
  // ========================================
  
  learningFromFailuresInstruction: "**重要：从以下失败经验中学习，避免重复错误**",
  
  failureRecordTitle: (index: number) => `### 失败记录 ${index + 1}`,
  
  failureReasonLabel: "原因",
  
  lessonsLabel: "教训",
  
  improvementsLabel: "改进建议",
  
  avoidRepeatMistakesReminder: "**特别注意避免上述失败记录中提到的问题**",
  
  applyImprovementsReminder: "应用失败记录中的改进建议",
  
  // ========================================
  // 任务调整提示词
  // ========================================
  
  adjustmentExpertRole: "你是一个任务调整专家。",
  
  adjustmentInstruction: "请根据质量评估结果生成任务树调整方案。",
  
  currentSubTasksTitle: "当前子任务：",
  
  qualityReviewResultTitle: "质量评估结果：",
  
  reviewStatusLabel: "状态",
  
  reviewDecisionLabel: "决策",
  
  findingsTitle: "发现的问题：",
  
  suggestionsTitle: "改进建议：",
  
  generateAdjustmentsInstruction: "请生成具体的调整方案，将改进建议转换为可执行的变更操作。",
  
  supportedChangeTypesTitle: "支持的变更类型：",
  
  changeTypes: {
    addTask: "add_task: 添加新的子任务",
    removeTask: "remove_task: 删除不必要的子任务",
    modifyTask: "modify_task: 修改子任务的描述或依赖关系",
    moveTask: "move_task: 移动子任务到新的父任务",
    mergeTasks: "merge_tasks: 合并多个子任务",
    splitTask: "split_task: 拆分子任务为多个子任务",
  },
  
  // ========================================
  // 🆕 V3: 总纲领生成提示词（Master Blueprint）
  // ========================================

  blueprintExpertRole: "你是一位资深的项目规划专家。",

  blueprintInstruction: `你的任务是为以下大型任务生成一份**完整、详细的总纲领（Master Blueprint）**。

这份纲领将作为"指挥家的总谱"，指导多个独立的执行者（每个执行者只看到自己的子任务 + 这份纲领）并行工作。
因此，纲领必须足够详细，让每个执行者即使不知道其他执行者的具体产出，也能保证：`,

  blueprintConsistencyPoints: [
    "内容风格统一",
    "角色/概念描述一致",
    "接口/衔接点明确",
    "不会出现矛盾或重复",
  ],

  blueprintTypeHints: {
    writing: `这是一个创作型任务。请生成完整的创作纲领，包括：
1. **世界观设定**：核心设定、背景规则、重要地点
2. **角色档案**：每个角色的性格特征、动机、成长弧线、关系网络
3. **剧情主线**：起承转合的完整大纲，关键转折点
4. **章节详细大纲**：每个章节/部分的：
   - 核心情节点（开场→发展→高潮→收尾）
   - 出场角色及其行动
   - 场景描写要点
   - 与前后章节的衔接点（"钩子"）
   - 目标字数和风格要求
5. **风格统一指南**：叙事视角、语言风格、氛围基调
6. **连续性标记**：章节间的伏笔、呼应、角色状态变化`,
    coding: `这是一个编码型任务。请生成完整的技术蓝图，包括：
1. **架构设计**：模块划分、接口定义、数据流
2. **每个模块的详细规格**：输入输出、依赖关系、关键函数签名
3. **实现顺序**：哪些模块可以并行开发，哪些有先后依赖
4. **质量标准**：每个模块的验收标准、测试要点`,
    research: `这是一个研究/调研型任务。请生成完整的研究纲领，包括：
1. **研究目标**：核心问题、预期产出物
2. **研究维度**：需要从哪些角度/层面进行调研
3. **信息来源**：每个维度的信息获取方式和优先级
4. **分析框架**：如何组织和对比收集到的信息
5. **交付标准**：每个部分的深度要求、字数/篇幅建议
6. **一致性要求**：术语定义、评价标准、格式规范`,
    data: `这是一个数据处理型任务。请生成完整的数据处理纲领，包括：
1. **数据源描述**：输入数据的格式、规模、质量情况
2. **处理流程**：每个处理步骤的输入输出、转换规则
3. **中间产物**：每个阶段的预期产出和格式要求
4. **质量校验**：数据一致性、完整性、准确性的检查方式
5. **最终产出**：交付物的格式、内容要求、验收标准`,
    design: `这是一个设计/架构型任务。请生成完整的设计纲领，包括：
1. **需求分析**：核心需求、约束条件、非功能性需求
2. **系统边界**：哪些在范围内、哪些在范围外
3. **架构方案**：整体架构图、核心组件职责、接口定义
4. **技术选型**：关键技术决策及其理由
5. **方案对比**：如有多个方案，列出各方案的优缺点和推荐理由
6. **验收标准**：每个设计产出的评审要点`,
    analysis: `这是一个分析型任务。请生成完整的分析纲领，包括：
1. **分析目标**：核心问题、预期结论类型
2. **分析维度**：需要从哪些角度进行分析
3. **方法论**：每个维度使用的分析方法（定量/定性）
4. **数据/材料要求**：每个维度需要的输入材料
5. **产出格式**：每部分分析的输出结构和深度要求
6. **综合结论**：如何从各维度分析中提炼最终结论`,
    generic: `请根据任务内容，生成完整的执行纲领，包括：
1. **总体目标**和成功标准
2. **分步规划**：每个子任务的详细描述和验收标准
3. **子任务间的依赖和协调关系**
4. **质量控制要点**`,
  },

  blueprintOriginalTaskLabel: "原始任务",

  blueprintOutputFormatHint: "请直接输出纲领内容（Markdown 格式），不要输出 JSON，不要解释你的思考过程。\n纲领应该详细到每个执行者只需要看自己的部分就能独立完成工作。",

  blueprintDecompositionInstruction: `📋 **总纲领（Master Blueprint）**：以下是为本任务预生成的详细规划纲领。
分解子任务时，每个子任务的 prompt 必须包含纲领中对应部分的详细要求（场景、角色、衔接点等）。
不要生成空泛的 prompt，要把纲领中的具体内容融入每个子任务。
同时，如果子任务间无数据依赖（如各章节创作），请将 dependencies 设为空数组 []，
这样系统可以并行执行它们以加快速度。`,

  blueprintChapterOutlineInstruction: `🔑 **重要**：每个子任务的 metadata 中必须包含 "chapterOutline" 字段，
将纲领中与该子任务对应的章节/模块大纲（场景、角色行动、情感节点、衔接点等）原样提取填入。
这个大纲会作为执行者的专属指导，确保并行执行时内容一致。`,

  blueprintTruncatedHint: "...[纲领已截断]",

  // 🆕 V7: 结构化写作纲领（多轮次生成）

  structuredBlueprintPass1Prompt: `你是一位资深的小说策划大师。请为以下创作任务构建**核心设定文档**，这是整部作品的基石。

请输出以下内容（Markdown 格式，不要 JSON）：

## 一、世界观设定
- 核心背景规则（力量体系/科技水平/社会结构等，视题材而定）
- 重要地点（至少 3 个关键场景，每个 2-3 句描述）
- 时间线/时代背景

## 二、风格指南
- 叙事视角（第几人称、是否切换视角）
- 语言风格（文风基调、节奏偏好、对话风格）
- 氛围基调（整体情绪走向）
- 禁忌事项（创作中应避免的问题）

## 三、主要人物卡
为每个主要角色（至少 3 个）创建详细档案：
- **姓名**
- **身份/职业**
- **性格标签**（3-5 个关键词）
- **核心动机**（驱动行为的内在需求）
- **成长弧线**（从开篇到结局的变化轨迹）
- **关键关系**（与其他角色的关系网络）
- **外貌标签**（2-3 个辨识特征）
- **口头禅/语言习惯**（如有）

请确保角色之间有张力和互补性，共同服务于主题。`,

  structuredBlueprintPass2Prompt: `你是一位资深的小说策划大师。基于以下已确定的**世界观和人物设定**，请为每个章节生成详细的剧情纲要。

请为每个章节输出以下结构（Markdown 格式）：

### 第N章：[章节标题]
- **核心情节**：本章的主要事件（3-5 个关键节点：开场→发展→高潮→收尾）
- **出场角色**：本章出场的角色及其行动/情感变化
- **关键场景**：主要场景描写要点
- **情感节点**：本章的情感高峰/低谷
- **衔接钩子**：
  - 承接上章：如何衔接前一章的结尾
  - 引出下章：本章结尾留下的悬念/伏笔
- **目标字数**：约 N 字

要求：
1. 每章情节必须推进主线，不能原地踏步
2. 角色行为必须符合人物卡中的性格和动机
3. 章节间的伏笔和呼应要明确标注
4. 控制节奏：动/静交替、张弛有度`,

  structuredBlueprintPass3Prompt: `你是一位资深的文学编辑。请审查以下创作纲领，检查并修正以下问题：

1. **人物一致性**：角色的行为/对话是否符合人物卡设定？成长弧线是否自然？
2. **情节逻辑**：事件因果关系是否成立？有无逻辑漏洞？
3. **节奏平衡**：是否有连续多章节奏过紧/过慢的问题？
4. **伏笔闭合**：所有伏笔是否有对应的揭示/回收？
5. **衔接缺口**：章节间的过渡是否自然？有无信息断层？

如果发现问题，请直接修正并输出完整的修正版纲领。
如果纲领质量良好，请输出"[审查通过]"后附上你认为可以进一步加强的建议（不超过 5 条）。`,

  structuredBlueprintOutputHint: "请直接输出内容（Markdown 格式），不要输出 JSON，不要解释你的思考过程。",

  // ========================================
  // 任务估算提示词
  // ========================================
  
  estimationExpertRole: "你是一个任务估算专家。",
  
  estimationInstruction: "请估算以下任务的复杂度和预计时长。",
  
  taskInfoTitle: "任务信息：",
  
  evaluationAspectsTitle: "请从以下几个方面评估任务：",
  
  complexityDescription: {
    title: "**复杂度**：",
    low: "low: 简单任务，可以快速完成",
    medium: "medium: 中等复杂度，需要一定时间",
    high: "high: 复杂任务，需要较长时间",
  },
  
  durationDescription: {
    title: "**预计时长**：",
    unit: "以毫秒为单位",
    considerations: "考虑任务的复杂度和工作量",
  },
  
  // ========================================
  // 质量评估提示词
  // ========================================
  
  qualityReviewExpertRole: "你是一个质量评估专家。",
  
  qualityReviewInstruction: "请评估以下任务树的质量。",
  
  reviewCriteriaTitle: "评估标准：",
  
  reviewCriteria: [
    "任务分解是否合理",
    "子任务是否覆盖了所有要求",
    "子任务之间的依赖关系是否正确",
    "任务粒度是否适中",
    "是否存在重复或冗余的任务",
  ],
  
  reviewDecisions: {
    title: "评估决策：",
    continue: "continue: 继续执行，质量良好",
    adjust: "adjust: 需要调整，但不需要重新分解",
    restart: "restart: 需要重新分解，保留失败经验",
    overthrow: "overthrow: 完全推翻，从头开始",
  },
  
  // 任务分解评估
  decompositionReview: {
    expertRole: "你是一个严格的任务质量评估专家，擅长评估任务分解方案的合理性。",
    instruction: "请严格评估以下任务分解的质量。特别关注：量化指标是否被正确分配到子任务、子任务间是否有并行执行的机会、以及分解粒度是否合理。",
    aspectsTitle: "请从以下几个方面评估任务分解的质量：",
    aspects: {
      coverage: "**覆盖性**：子任务是否完整覆盖了根任务的所有要求？是否有遗漏的关键步骤？",
      independence: "**独立性**：每个子任务是否可以独立执行？无依赖的任务是否可以并行？",
      granularity: "**合理性**：子任务的粒度是否合理？是否过于细碎或过于粗糙？",
      dependencies: "**依赖性**：子任务之间的依赖关系是否合理？是否有不必要的串行依赖阻碍并行执行？",
      completeness: "**完整性**：是否有遗漏的重要步骤？",
      redundancy: "**冗余性**：是否有重复或不必要的子任务？",
      quantitative: "**量化分配**：如果根任务有字数/数量等量化要求，是否已合理分配到各子任务？各子任务的量化指标之和是否 >= 总要求？",
    },
  },
  
  // 子任务完成评估
  completionReview: {
    expertRole: "你是一个严格的任务质量评估专家，擅长多维度深度审查。",
    instruction: "请从多个维度严格评估以下子任务的完成质量。特别注意：如果任务有明确的字数/篇幅要求，必须核实实际产出是否达标。",
    aspectsTitle: "请从以下几个方面评估子任务的完成质量：",
    aspects: {
      completeness: "**完成度**：是否完成了任务描述中的所有要求？是否有遗漏的子项？",
      correctness: "**正确性**：输出内容是否准确？逻辑是否自洽？是否有事实性错误？",
      integrity: "**完整性**：内容是否完整？结构是否完整（有开头有结尾）？是否有明显的截断或草草收尾？",
      quality: "**质量**：输出的质量如何？文笔/代码质量是否达到预期水准？",
      quantitative: "**量化达标**：如果任务要求了具体的字数、篇幅、数量等量化指标，实际产出是否达到要求的 70% 以上？低于 70% 必须判定 restart。请估算实际字数并与要求对比。",
      coherence: "**连贯性**：内容前后是否连贯？风格是否统一？是否有突兀的跳跃或矛盾？",
    },
  },
  
  // 整体完成评估
  overallReview: {
    expertRole: "你是一个严格的任务质量评估专家，擅长从全局视角审查任务完成情况。",
    instruction: "请从全局视角严格评估以下任务的整体完成质量。重点关注：量化指标是否达标、各子任务输出是否协调一致、是否有明显的质量短板。",
    aspectsTitle: "请从以下几个方面评估整体完成质量：",
    aspects: {
      goalAchievement: "**目标达成**：是否达成了根任务的目标？量化指标（字数、数量等）是否达标？",
      completeness: "**完整性**：是否有遗漏的重要内容？所有子任务的产出合在一起是否构成完整的交付物？",
      consistency: "**一致性**：各个子任务的输出风格是否统一？是否有矛盾或冲突？术语和命名是否一致？",
      quality: "**质量**：整体质量如何？是否有明显的质量短板拉低整体水平？",
      coherence: "**连贯性**：各部分之间的衔接是否自然？是否有突兀的断裂或重复？",
    },
  },
  
  // 失败分析
  failureAnalysis: {
    expertRole: "你是一个任务失败分析专家。",
    instruction: "请分析以下子任务失败的原因。",
    aspectsTitle: "请从以下几个方面分析失败原因：",
    aspects: {
      directCause: "**直接原因**：导致失败的直接原因是什么？",
      rootCause: "**根本原因**：失败的根本原因是什么？",
      lessons: "**教训**：从这次失败中可以学到什么？",
      improvements: "**改进建议**：如何避免类似的失败？",
    },
    decisionsTitle: "决策说明：",
    decisions: {
      adjust: "adjust: 小问题，调整后重试",
      restart: "restart: 严重问题，需要保留经验并重新分解",
      overthrow: "overthrow: 根本性错误，需要完全推翻重来",
    },
  },
  
  // 通用标签
  labels: {
    rootTask: "根任务",
    subTaskList: "子任务列表",
    subTaskInfo: "子任务信息",
    completedSubTasks: "已完成的子任务",
    errorInfo: "错误信息",
    reviewStatus: "状态",
    reviewDecision: "决策",
    findings: "发现的问题",
    suggestions: "改进建议",
    evaluationCriteria: "评估标准",
    description: "描述",
    status: "状态",
    output: "输出",
    noOutput: "无",
    reviewTypeLabels: {
      initial_decomposition: "初始任务分解",
      subtask_completion: "子任务完成",
      overall_completion: "整体完成",
      failure_analysis: "失败分析",
    },
    qualityStatusLabels: {
      pending: "待评估",
      passed: "通过",
      partial: "部分完成",
      needs_adjustment: "需要调整",
      needs_restart: "需要重启",
      needs_overthrow: "需要推翻",
    },
    reviewDecisionLabels: {
      continue: "继续执行",
      adjust: "调整任务树",
      restart: "重启任务",
      overthrow: "推翻任务",
      decompose: "增量分解",
    },
    changeTypeLabels: {
      add_task: "添加任务",
      remove_task: "删除任务",
      modify_task: "修改任务",
      move_task: "移动任务",
      merge_tasks: "合并任务",
      split_task: "拆分任务",
    },
  },
  
  // ========================================
  // 任务调整器提示词
  // ========================================
  taskAdjuster: {
    errors: {
      changeValidationFailed: "变更验证失败",
      targetTaskNotFound: "目标任务不存在",
      addTaskMissingAfter: "add_task 变更缺少 after 字段",
      modifyTaskMissingAfter: "modify_task 变更缺少 after 字段",
      moveTaskMissingParentId: "move_task 变更缺少 after.parentId 字段",
      noTasksToMerge: "没有找到要合并的任务",
      taskNotFound: "任务不存在",
      unknownChangeType: "未知的变更类型",
      applyChangeFailed: "应用变更失败",
    },
    logs: {
      retryCountExceeded: "任务重试次数过多，建议拆分为更小的子任务",
      improvementSuggestion: "改进建议",
    },
  },
  
  // ========================================
  // 恢复管理器提示词
  // ========================================
  recoveryManager: {
    logs: {
      restoredFromCheckpoint: "✅ 从检查点恢复",
      failedToRestore: "⚠️ 从检查点恢复失败",
      recoveredTaskTree: "✅ 恢复任务树，包含 {count} 个中断的任务",
      reexecutingTasks: "🔄 重新执行 {count} 个中断的任务",
      reexecutingTask: "🔄 重新执行任务: {id} ({summary})",
    },
  },
  
  // ========================================
  // 重试管理器提示词
  // ========================================
  retryManager: {
    logs: {
      executingTask: "🔄 执行任务 (尝试 {attempt}/{maxRetries}): {id}",
      taskSucceeded: "✅ 任务成功: {id}",
      taskFailed: "❌ 任务失败 (尝试 {attempt}/{maxRetries}): {id}",
      errorNotRetryable: "⚠️ 错误不可重试: {message}",
      waitingBeforeRetry: "⏳ 等待 {delay}ms 后重试...",
      allRetriesFailed: "❌ 所有重试都失败了: {id}",
      failureLogged: "📝 失败日志已记录: {path}",
    },
  },
  
  // ========================================
  // 🆕 V5: 大文本 Map-Reduce 分析提示词
  // ========================================
  mapReduce: {
    mapTitle: (chunkIndex: number, totalChunks: number) =>
      `【分段阅读分析 — 第 ${chunkIndex}/${totalChunks} 段】`,
    mapReadFileInstruction: "请使用 read 工具读取文件：",
    mapLineRangeInstruction: "行范围：",
    mapAnalysisGoalIntro: "阅读后，按以下目标进行分析：",
    mapImportantTitle: "⚠️ 重要：",
    mapMustReadFirst: "必须先用 read 工具读取上述文件的指定行范围",
    mapWriteToFile: "将分析结果写入文件（使用 write 工具）",
    mapFileNameFormat: "文件名格式：",
    mapResultContents: "分析结果应包含：关键发现、摘要、与分析目标相关的具体内容摘录",
    mapDeepReadHint: `如果本段中有需要更细致分析的重要段落，请在输出末尾标注"[需要精读]"并注明行号范围`,
    mapOverlapNote: (overlapLines: number) =>
      `注意：本段开头 ${overlapLines} 行与上一段重叠，避免重复分析`,

    reduceTitle: (batchIndex: number, totalBatches: number) =>
      `【整合分析 — 批次 ${batchIndex}/${totalBatches}】`,
    reduceReadIntro: "请读取以下分段分析结果并进行整合：",
    reduceGoalIntro: "整合目标（与原始任务一致）：",
    reduceRequirementsTitle: "⚠️ 要求：",
    reduceReadFiles: "使用 read 工具读取上述分析文件",
    reduceDedup: "去重、合并、提炼，写入整合报告",
    reduceSaveTo: "保存到",
    reduceKeepFindings: "保留关键发现和具体摘录，去除重复内容",

    finalizeTitle: "【最终产出】",
    finalizeReadIntroFromReduce: "请读取以下整合分析报告并生成最终交付物：",
    finalizeReadIntroFromMap: "请读取以下分段分析结果并生成最终交付物：",
    finalizeGoalIntro: "最终产出目标（与原始任务一致）：",
    finalizeRequirementsTitle: "⚠️ 要求：",
    finalizeReadFiles: "使用 read 工具读取上述文件",
    finalizeSynthesize: "综合所有分析结果，生成完整、结构化的最终交付物",
    finalizeWriteOutput: "使用 write 工具保存最终产出",
    finalizeSaveTo: "保存到",
    finalizeEnsureComplete: "确保最终产出完整、无遗漏、结构清晰",

    chunkFileTypeHint: "文件",
    defaultFileTypeHint: ".txt 文件",
  },

  // ========================================
  // 质量评审器提示词
  // ========================================
  qualityReviewer: {
    errors: {
      reviewFailed: "质量评估失败",
      subTaskNotFound: "子任务不存在",
      completionReviewFailed: "子任务完成质量评估失败",
      overallReviewFailed: "整体完成质量评估失败",
      failureAnalysisFailed: "失败分析失败",
      saveRecordFailed: "保存质量评估记录失败",
    },
    report: {
      title: "# 质量评估报告",
      taskTreeId: "**任务树 ID**",
      reviewType: "**评估类型**",
      reviewTime: "**评估时间**",
      reviewStatus: "**评估状态**",
      reviewDecision: "**评估决策**",
      criteriaTitle: "## 评估标准",
      findingsTitle: "## 发现的问题",
      suggestionsTitle: "## 改进建议",
      changesTitle: "## 应用的变更",
      changeTarget: "目标",
    },
  },
};
