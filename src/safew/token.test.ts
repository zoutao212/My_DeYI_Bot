...
function withTempDir(): string {
  return fs.mkdtempSync(path.join(os.tmpdir(), "clawdbot-safew-token-"));
}

describe("resolveSafewToken", () => {
  afterEach(() => {
    vi.unstubAllEnvs();
  });

  it("prefers config token over env", () => {
    vi.stubEnv("SAFEW_BOT_TOKEN", "env-token");
    const cfg = {
      channels: { safew: { botToken: "cfg-token" } },
    } as ClawdbotConfig;
    const res = resolveSafewToken(cfg);
  ...
  it("uses tokenFile when configured", () => {
    vi.stubEnv("SAFEW_BOT_TOKEN", "");
    const dir = withTempDir();
    const tokenFile = path.join(dir, "token.txt");
    fs.writeFileSync(tokenFile, "file-token\n", "utf-8");
    const cfg = { channels: { safew: { tokenFile } } } as ClawdbotConfig;
    const res = resolveSafewToken(cfg);
  ...
  it("does not fall back to config when tokenFile is missing", () => {
    ...
    const cfg = {
      channels: { safew: { tokenFile, botToken: "cfg-token" } },
    } as ClawdbotConfig;
    const res = resolveSafewToken(cfg);
