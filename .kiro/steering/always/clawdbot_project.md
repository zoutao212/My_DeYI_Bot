...
2.1 Orchestrator（任务协调器）- 中枢大脑
  ...
  主要方法：
    ...
    ★ getNextExecutableTasksForDrain() - [方案A] 任务树驱动的drain调度决策（返回DrainScheduleResult）
    ★ onTaskStarting()                 - [V2] 任务启动前准备（构建ExecutionContext+自动分解判断）
    ★ onTaskCompleted()                - [V2] 任务完成后统一后处理（postProcess+轮次完成检查）
    ★ onTaskFailed()                   - [V2] 任务失败后集中处理（重试/级联/停止）
    ★ onRoundCompleted()               - [V2] 轮次完成后处理（合并输出+交付报告+归档）
...
2.3 followup-runner - 执行引擎
  ...
  核心流程（修复后）：
    ...
    11) ★[P0+P1] 调用 postProcessSubTaskCompletion()（统一质检+决策响应+文件验证+持久化）
        - restart → 重置 pending + 创建新 FollowupRun 入队 + finalizeWithFollowup() 触发 drain
...
2.4 drain.ts - 队列调度器
  文件：drain.ts（~260行）
  ★ 方案 A：任务树驱动调度（替代旧的守卫 A/B/C/D + FIFO）
  核心原则：任务树是唯一真相源，drain 只负责匹配 FollowupRun 并执行
  调度流程：
    orchestrator.getNextExecutableTasksForDrain(taskTree, roundId)
    → DrainScheduleResult（5 种 action）：
      execute      → 从队列匹配 FollowupRun，串行/并行执行
      wait         → pending 任务都在等依赖/兄弟，短暂等待 2s
      round_done   → 清理该轮次残留队列项
      discard_all  → 任务树已终结，丢弃所有队列任务
      discard_round→ 轮次被 overthrow，级联丢弃
  兜底：任务树查询失败时回退到 FIFO shift
  并行执行：
    任务树返回多个可执行任务时 → Promise.allSettled() 并发
...
2.6 QualityReviewer - 质检官
  ...
  决策：continue / adjust / restart / overthrow / decompose(增量分解)
...
阶段2：任务分解（可选的递归分解）
  ...
  ★ [P2修复后] 递归分解已在 followup-runner 中自动触发：
    ...
    分解成功后为每个 pending 子任务创建 FollowupRun 入队，然后跳过直接执行。
    ...
    scheduleFollowupDrain(key, runFollowup)
      -> drain while循环
        ...
        -> switch(schedule.action):
            execute      → 匹配 FollowupRun，串行/并行执行
            wait         → 短暂等待 2s（pending 任务都在等依赖/兄弟）
            round_done   → 清理轮次残留
            discard_all  → 丢弃所有（任务树已终结）
            discard_round→ 级联丢弃（轮次被 overthrow）
        -> FIFO 兜底（任务树查询失败时）
...
阶段4：质量评估
  ...
  调用链（P0+P1修复后）：
    ...
    结果处理（决策已真正生效）：
      ...
      restart   -> 重置 pending + retryCount++ + needsRequeue=true → followup-runner 创建新 FollowupRun 入队
