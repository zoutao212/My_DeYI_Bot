...
## 六、补丁工具（apply_patch）反复失败时的止损流程（强制）

适用：出现以下报错之一：

- `The string to replace was not found in the file.`
- `Could not successfully apply any edits.`
- “连续失败 2 次，要求 EXTREME SUSPICION REQUIRED”

### 6.1 根因速判（本质只有一类）

- **apply_patch 是“精确文本匹配”**：上下文/空格/缩进/反斜杠/换行符任何一处不一致，都可能导致找不到替换片段。

高频踩坑（本次已复现）：

- **路径反斜杠误写**：文件里是 `c:\Users\<user>\...` 还是 `c:\Users\<user>\...`（单斜杠/双斜杠、转义差异）必须逐字符一致。
- **文件末尾空行**：Markdown 末尾多/少一个空行会让补丁上下文不命中。
- **补丁上下文不唯一或不够长**：同一段文字多处出现，容易打到错误位置或直接拒绝。

### 6.2 强制止损步骤（必须按顺序执行）

1. **立即停止继续盲改**：不要连续试“差不多”的 patch。
2. **read_file 精确读取目标区域**：
   - 优先读取“目标块 + 上下各 5-15 行”。
   - 若是文件末尾问题，直接读取整个文件或最后 40 行。
3. **复制粘贴原文做 patch 上下文**：
   - patch 里的未修改行必须与文件 **完全一致**（含缩进、反斜杠、空格）。
4. **缩小修改面**：一次 patch 只做一件事（例如只删末尾多余空行）。
5. **改完立刻复核闭环**：
   - 再 `read_file` 或 `grep` 确认目标变化真实落盘。

### 6.3 连续失败 2 次后的额外要求（强制）

- 在下一次 patch 前必须先输出三段说明：
  1) 逐条解释前两次失败的原因（具体到哪个字符/哪行不一致）。
  2) 为什么这次会成功（指出你已 read_file 并逐字对齐）。
  3) 对即将修改的文本做“逐字符复核”（尤其是路径反斜杠与空行）。
